/*!
 * QUnit 2.15.0-pre 2021-05-27T16:56Z
 * https://qunitjs.com/
 *
...
(function () {
  ...
  */
  ...
  Test.prototype = {
    ...
    run: function run() {
      ...
      function runTest(test) {
        var promise = test.callback.call(test.testEnvironment, test.assert, test.params);
  ...
  function _todo(testName, data, callback) {
    if (focused || config.currentModule.ignored) {
      return;
    }
  ...
  function _only(testName, data, callback) {
  ...
  function runEach(data, eachFn) {
    if (Array.isArray(data)) {
      ...
      throw new Error("test.each expects an array of arrays or an array of primitives as the expected input.\n".concat(_typeof(data), " was found instead."));
    }
  }

  extend(test, {
    todo: function todo(testName, callback) {
      _todo(testName, undefined, callback);
    },
    skip: skip,
    only: function only(testName, callback) {
      _only(testName, undefined, callback);
    },
    each: function each(testName, data, callback) {
      runEach(data, function (datum, i) {
        addTestWithData({
  ...
  test.todo.each = function (testName, data, callback) {
    runEach(data, function (datum, i) {
      _todo(makeEachTestName(testName, i), datum, callback);
    });
  };

  test.skip.each = function (testName, data) {
    runEach(data, function (_, i) {
  ...
  test.only.each = function (testName, data, callback) {
    runEach(data, function (datum, i) {
      _only(makeEachTestName(testName, i), datum, callback);
    });
  ...
  QUnit.version = "2.15.0-pre 2021-05-27T16:56Z";
